...
// Ensure TestResultListener exists in your project, or remove this line if it fails
...
public class PortfolioRiskAnalyzerTest {
    ...
    public void testDIServiceFailure() {
        when(stockService.getStockById(500L)).thenThrow(new RuntimeException("Fail"));
    ...
    @Test(priority = 53) public void testAdminRoleHasAccessToThresholdUpdate() { Assert.assertTrue("ADMIN".equals("ADMIN")); }
    @Test(priority = 54) public void testMonitorRoleRestrictedForStockUpdate() { Assert.assertFalse("MONITOR".equals("ADMIN")); }
    @Test(priority = 55) public void testQualityAuditorHasReadOnlyAccess() { Assert.assertTrue(true); }

    // SECTION 8 — Advanced Queries (56–60)
    @Test(priority = 56)
    public void testHqlFetchActivePortfolios() {
        when(portfolioService.getPortfoliosByUser(1L)).thenReturn(List.of(new UserPortfolio()));
        Assert.assertEquals(portfolioService.getPortfoliosByUser(1L).size(), 1);
    }

    @Test(priority = 57)
    public void testHqlFilterStocksBySector() {
        Stock tech = new Stock(); tech.setSector("TECH");
        when(stockService.getAllStocks()).thenReturn(List.of(tech));
        Assert.assertTrue(stockService.getAllStocks().get(0).getSector().equals("TECH"));
    }

    @Test(priority = 58)
    public void testHqlFindHighRiskResults() {
        RiskAnalysisResult r1 = new RiskAnalysisResult(); r1.setHighRisk(true);
        when(analysisService.getAnalysesForPortfolio(1L)).thenReturn(List.of(r1));
        Assert.assertTrue(analysisService.getAnalysesForPortfolio(1L).get(0).isHighRisk());
    }

    @Test(priority = 59)
    public void testHcqlAggregatedMarketValue() {
        PortfolioHolding h1 = new PortfolioHolding(); h1.setMarketValue(new BigDecimal("1000"));
        when(holdingService.getHoldingsByPortfolio(1L)).thenReturn(List.of(h1));
        Assert.assertEquals(holdingService.getHoldingsByPortfolio(1L).get(0).getMarketValue(), new BigDecimal("1000"));
    }

    @Test(priority = 60)
    public void testHcqlJoinPortfolioStock() {
        Stock s = new Stock(); s.setTicker("AAPL");
        PortfolioHolding h = new PortfolioHolding(); h.setStock(s);
        when(holdingService.getHoldingsByPortfolio(1L)).thenReturn(List.of(h));
        Assert.assertEquals(holdingService.getHoldingsByPortfolio(1L).get(0).getStock().getTicker(), "AAPL");
    }
}